...
namespace Ingogo.MVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            var user = c.Users.ToList().Find(x => x.Email == model.Email);
            ...
            //if (owner.Identifier("ingogofarm@outlook.com") == 1 || model.Email == "ingogofarm@outlook.com" && model.Password == "Ingogo2015")
            //{
            //    return RedirectToAction("AddOwner", "Owners");
            //}

            switch (result)
            {
                case SignInStatus.Success:
                    //if (user.UserProfile.IsFirstTimeLogin == true && model.Email == "ingogofarm@outlook.com")
                    //{
                    //    return RedirectToAction("UpdateOwner", "Owners");
                    //}
                    if (UserManager.IsInRole(user.Id, "Owner"))
                    {
                        return RedirectToAction("OwnerHome", "Owner");
                    }
                    if (user.UserProfile.IsFirstTimeLogin == true)
                    {
                        return RedirectToAction("ChangePassword", "Manage");
                    }
                   
                    if (UserManager.IsInRole(user.Id, "Farm Manager"))
                    {
                        return RedirectToAction("FarmManagerHome", "FarmManager");
                    }
                    if (UserManager.IsInRole(user.Id, "Supervisor"))
                    {
                        return RedirectToAction("SupervisorHome", "Supervisor");
                    }
                    if (UserManager.IsInRole(user.Id, "Admin Clerk"))
                    {
                        return RedirectToAction("AdminClerkHome", "AdminClerk");
                    }
                    if (UserManager.IsInRole(user.Id, "Farm Assistance"))
                    {
                        return RedirectToAction("FarmAssistanceHome", "FarmAssistance");
                    }
                    return RedirectToAction("BadRequest", "Account");
